...
IMAGE_NAME="${CLAWDBOT_IMAGE:-rgbot:local}"
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm rgbot-cli onboard --no-install-daemon

echo ""
echo "==> Provider setup (optional)"
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm rgbot-cli providers login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm rgbot-cli providers add --provider telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm rgbot-cli providers add --provider discord --token <token>"
echo "Docs: https://docs.molt.bot/providers"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d rgbot-gateway
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f rgbot-gateway"
echo "  ${COMPOSE_HINT} exec rgbot-gateway node dist/index.js health --token \"$CLAWDBOT_GATEWAY_TOKEN\""
