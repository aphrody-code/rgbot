...
ZIP=${1:?"Usage: $0 Rose Griffon Bot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/rgbot/rgbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Rose Griffon Bot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/rgbot/rgbot/releases/download/v${VERSION}/"}
