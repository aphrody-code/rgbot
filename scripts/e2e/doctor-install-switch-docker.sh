...
IMAGE_NAME="rgbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/rgbot-bin:$PATH"
  mkdir -p /tmp/rgbot-bin

  cat > /tmp/rgbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/rgbot-bin/systemctl

  cat > /tmp/rgbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/rgbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/rgbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/rgbot/rgbot.mjs"
  git_entry="/app/rgbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/rgbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/rgbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
