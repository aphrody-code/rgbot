...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/rgbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/rgbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/rgbot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.clawdbot/rgbot.json"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.clawdbot/rgbot.json" <<'"'"'JSON'"'"'
{
