#!/bin/bash
# Setup Rose Griffon Bot Auth Management System
...
echo "=== Rose Griffon Bot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'rgbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Rose Griffon Bot message: Send warning via Rose Griffon Bot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/rgbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/rgbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/rgbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now rgbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status rgbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
