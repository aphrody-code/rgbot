...
# Checks both Claude Code and Rose Griffon Bot auth status
...
fetch_models_status_json() {
    rgbot models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    rgbot_status=$(check_rgbot_auth 2>/dev/null || true)

    claude_expires=0
    rgbot_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        rgbot_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        rgbot_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CLAWDBOT_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$rgbot_status" \
        --arg be "$rgbot_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            rgbot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$rgbot_status" == EXPIRED* ]] || [[ "$rgbot_status" == MISSING* ]]; then
        echo "CLAWDBOT_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$rgbot_status" == EXPIRING* ]]; then
...
echo ""
echo "Rose Griffon Bot Auth (~/.clawdbot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'rgbot doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active rgbot >/dev/null 2>&1; then
    echo -e "Rose Griffon Bot service: ${GREEN}running${NC}"
else
    echo -e "Rose Griffon Bot service: ${RED}NOT running${NC}"
fi
