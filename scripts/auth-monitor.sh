...
#
# Suggested cron: */30 * * * * /home/admin/rgbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Rose Griffon Bot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., rgbot-alerts)
...
send_notification() {
    ...
    # Send via Rose Griffon Bot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use rgbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Rose Griffon Bot to $NOTIFY_PHONE..."
            rgbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: Rose Griffon Bot Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Rose Griffon Bot is down. Run: ssh l36 '~/rgbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
