...
BASE_IMAGE="${BASE_IMAGE:-rgbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-rgbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=rgbot.sandbox=1)
NOTE
