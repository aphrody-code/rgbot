...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/rgbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/rgbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/rgbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CLAWDBOT_CONTROL_UI_BASE_PATH__ = "/rgbot";
    const app = mountApp("/rgbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/rgbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/rgbot/sessions");
  });
