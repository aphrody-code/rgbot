...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as Rose Griffon BotApp);
}
...
async function sendChatMessageNow(
  ...
  const ok = await sendChatMessage(host as unknown as Rose Griffon BotApp, message, opts?.attachments);
...
export async function refreshChat(host: ChatHost) {
  await Promise.all([
    loadChatHistory(host as unknown as Rose Griffon BotApp),
    loadSessions(host as unknown as Rose Griffon BotApp, { activeMinutes: 0 }),
