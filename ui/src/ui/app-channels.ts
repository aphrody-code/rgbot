...
export async function handleWhatsAppStart(host: Rose Griffon BotApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: Rose Griffon BotApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: Rose Griffon BotApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: Rose Griffon BotApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: Rose Griffon BotApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: Rose Griffon BotApp): string {
...
export function handleNostrProfileCancel(host: Rose Griffon BotApp) {
...
export function handleNostrProfileToggleAdvanced(host: Rose Griffon BotApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: Rose Griffon BotApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: Rose Griffon BotApp) {
  const state = host.nostrProfileFormState;
