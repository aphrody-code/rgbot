...
function candidateBinDirs(opts: EnsureRgBotPathOpts): string[] {
  ...
  // Bundled macOS app: `rgbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingRgBot = path.join(execDir, "rgbot");
    if (isExecutable(siblingRgBot)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/rgbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "rgbot"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `rgbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureRgBotCliOnPath(opts: EnsureRgBotPathOpts = {}) {
