...
export const WIDE_AREA_DISCOVERY_DOMAIN = "rgbot.internal.";
export const WIDE_AREA_ZONE_FILENAME = "rgbot.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*rgbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "rgbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "rgbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "rgbot-gw");
  ...
  records.push(`_rgbot-gw._tcp IN PTR ${instanceLabel}._rgbot-gw._tcp`);
  records.push(`${instanceLabel}._rgbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._rgbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; rgbot-content-hash: ${contentHash}\n${contentBody}`;
}
