...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveRgBotPackageRoot } = await import("./rgbot-root.js");
    ...
    vi.mocked(resolveRgBotPackageRoot).mockResolvedValue("/opt/rgbot");
