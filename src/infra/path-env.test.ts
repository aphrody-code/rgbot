...
import { ensureRgBotCliOnPath } from "./path-env.js";

describe("ensureRgBotCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling rgbot exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "rgbot");
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "rgbot");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "rgbot");
