...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: RgBotConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: RgBotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: RgBotConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: RgBotConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: RgBotConfig): void {
