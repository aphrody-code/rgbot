...
// // // import { /* parseIMessageTarget */ ({} as any), /* normalizeIMessageHandle */ ((s:string)=>s) } from "../../imessage/targets.js";
import {
...
function resolveWhatsAppSession(
  ...
  const normalized = /* normalizeWhatsAppTarget */ ((s:string)=>s)(params.target);
  if (!normalized) return null;
  const isGroup = /* isWhatsAppGroupJid */ ((_s:string)=>false)(normalized);
  const peer: RoutePeer = {
