...
import { createRgBotTools } from "../agents/rgbot-tools.js";
import {
  filterToolsByPolicy,
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-rgbot-message-channel") ?? "");
  const accountId = getHeader(req, "x-rgbot-account-id")?.trim() || undefined;
  ...
  const allTools = createRgBotTools({
