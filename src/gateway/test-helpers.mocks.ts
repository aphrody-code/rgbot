...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `rgbot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.CLAWDBOT_CONFIG_PATH = path.join(root, "rgbot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "rgbot.json");
