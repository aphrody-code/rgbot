...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "RgBot";
  if (/rgbot/i.test(trimmed)) return trimmed;
  return `${trimmed} (RgBot)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const siblingCli = path.join(execDir, "rgbot");
  ...
  const binCli = path.join(cwd, "bin", "rgbot.js");
