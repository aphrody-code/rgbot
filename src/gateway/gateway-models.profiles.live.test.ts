...
import { resolveRgBotAgentDir } from "../agents/agent-paths.js";
...
import { ensureRgBotModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { RgBotConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<RgBotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveRgBotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-live-state-"));
  ...
  const agentDir = resolveRgBotAgentDir();
  const sanitizedCfg: RgBotConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-live-"));
  const tempConfigPath = path.join(tempDir, "rgbot.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CLAWDBOT_CONFIG_PATH = tempConfigPath;

  await ensureRgBotModelsJson(nextCfg);
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "RgBot live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureRgBotModelsJson(cfg);
