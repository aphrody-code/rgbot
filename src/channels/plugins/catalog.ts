...
import { discoverRgBotPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { RgBotPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  rgbot?: RgBotPackageManifest;
  [LEGACY_MANIFEST_KEY]?: RgBotPackageManifest;
};
...
function toChannelMeta(params: {
  channel: NonNullable<RgBotPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageRgBot?: RgBotPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageRgBot;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.rgbot ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageRgBot: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverRgBotPlugins({ workspaceDir: options.workspaceDir });
