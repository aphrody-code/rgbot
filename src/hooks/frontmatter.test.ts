...
`;
    ...
    expect(parsed.rgbot.emoji).toBe("💾");
    expect(parsed.rgbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.rgbot.emoji).toBe("📝");
    expect(parsed.rgbot.events).toEqual(["command"]);
    expect(parsed.rgbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.rgbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"rgbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"rgbot": {"events": ["test"]}}');
  });
...
describe("resolveRgBotMetadata", () => {
  it("extracts rgbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveRgBotMetadata(frontmatter);
  ...
  it("returns undefined when rgbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        rgbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with RgBot" },
            { id: "npm", kind: "npm", package: "@rgbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@rgbot/hook");
  });
    ...
    "rgbot":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with RgBot" }],
      },
  }
---
...
`;
    ...
    const rgbot = resolveRgBotMetadata(frontmatter);
    expect(rgbot).toBeDefined();
    expect(rgbot?.emoji).toBe("💾");
    expect(rgbot?.events).toEqual(["command:new"]);
    expect(rgbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(rgbot?.install?.[0].kind).toBe("bundled");
  });
