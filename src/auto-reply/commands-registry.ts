...
export function isCommandEnabled(cfg: RgBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: RgBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as RgBotConfig),
...
export function getCommandDetection(_cfg?: RgBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: RgBotConfig) {
