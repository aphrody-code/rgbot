...
type AgentConfig = Partial<NonNullable<NonNullable<RgBotConfig["agents"]>["defaults"]>>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 RgBot ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: RgBotConfig): string {
  const lines = ["ℹ️ Help", ""];
