...
import { scheduleGatewaySigusr1Restart, triggerRgBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting rgbot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerRgBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting rgbot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
