...
function resolveMainSessionKeyForSandbox(params: { cfg?: RgBotConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: RgBotConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`rgbot sandbox explain --session ${runtime.sessionKey}`)}`,
  );
