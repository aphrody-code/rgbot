...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "rgbot.sandbox=1");
  args.push("--label", `rgbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `rgbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `rgbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "rgbot.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`rgbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`rgbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("rgbot sandbox recreate --all");
}
