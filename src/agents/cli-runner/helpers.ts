...
/**
 * Cleanup suspended RgBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: RgBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-cli-images-"));
  const paths: string[] = [];
