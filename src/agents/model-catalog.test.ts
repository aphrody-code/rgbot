...
vi.mock("./models-config.js", () => ({
  ensureRgBotModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
}));

vi.mock("./agent-paths.js", () => ({
  resolveRgBotAgentDir: () => "/tmp/rgbot",
}));
...
describe("loadModelCatalog", () => {
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as RgBotConfig });
