...
const defaultTools = createRgBotCodingTools();

describe("createRgBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createRgBotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from rgbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "rgbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "rgbot-workspace"),
    ...
    const tools = createRgBotCodingTools({ sandbox });
