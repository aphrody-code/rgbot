...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("rgbot-ws-", async (workspaceDir) => {
      await withTempDir("rgbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createRgBotCodingTools({ workspaceDir });
    ...
    await withTempDir("rgbot-ws-", async (workspaceDir) => {
      await withTempDir("rgbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello rgbot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("rgbot-ws-", async (workspaceDir) => {
      await withTempDir("rgbot-override-", async (overrideDir) => {
        const tools = createRgBotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("rgbot-sandbox-", async (sandboxDir) => {
      await withTempDir("rgbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "rgbot-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "rgbot-sandbox:bookworm-slim",
            containerPrefix: "rgbot-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createRgBotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
