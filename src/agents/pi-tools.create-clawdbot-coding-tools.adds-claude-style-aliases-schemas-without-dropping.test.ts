...
import { createRgBotTools } from "./rgbot-tools.js";
import { __testing, createRgBotCodingTools } from "./pi-tools.js";
...
describe("createRgBotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createRgBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "rgbot-outside.txt");
