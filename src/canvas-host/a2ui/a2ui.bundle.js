...
var Rose Griffon BotA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--rgbot-a2ui-inset-top, 0px)
        var(--rgbot-a2ui-inset-right, 0px)
        var(--rgbot-a2ui-inset-bottom, 0px)
        var(--rgbot-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--rgbot-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--rgbot-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--rgbot-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--rgbot-a2ui-empty-top, var(--rgbot-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		globalThis.rgbotA2UI = api;
		globalThis.clawdbotA2UI = api;
		this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
		this.#statusListener = (evt) => this.#handleActionStatus(evt);
		globalThis.addEventListener("rgbot:a2ui-action-status", this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			globalThis.removeEventListener("rgbot:a2ui-action-status", this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__rgbotLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.rgbotCanvasA2UIAction ?? globalThis.webkit?.messageHandlers?.clawdbotCanvasA2UIAction ?? globalThis.rgbotCanvasA2UIAction ?? globalThis.clawdbotCanvasA2UIAction;
		if (handler?.postMessage) {
			try {
				if (handler === globalThis.rgbotCanvasA2UIAction || handler === globalThis.clawdbotCanvasA2UIAction) {
...
customElements.define("rgbot-a2ui-host", Rose Griffon BotA2UIHost);
