...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("rgbotCanvasA2UIAction");
    expect(out).toContain("rgbotSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("rgbotSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.rgbotA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__rgbot__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("rgbot-a2ui-host");
      expect(html).toContain("rgbotCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__rgbot__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("rgbotA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
