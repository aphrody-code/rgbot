...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.rgbotCanvasA2UIAction ||
        window.webkit.messageHandlers.clawdbotCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.rgbotCanvasA2UIAction &&
        typeof window.rgbotCanvasA2UIAction.postMessage === "function") ||
  ...
  if (!window.rgbotSendUserAction && legacySend) {
    window.rgbotSendUserAction = legacySend;
  }
  if (!window.clawdbotSendUserAction && typeof window.rgbotSendUserAction === "function") {
    window.clawdbotSendUserAction = window.rgbotSendUserAction;
  }
  const hasHelper = () =>
    typeof window.rgbotSendUserAction === "function" ||
  ...
  window.addEventListener("rgbot:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android RgBot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.rgbotSendUserAction === "function"
        ? window.rgbotSendUserAction
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>RgBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
