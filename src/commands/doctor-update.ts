...
async function detectRgBotGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const git = await detectRgBotGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update RgBot from git before running doctor?",
      initialValue: true,
    });
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("rgbot update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
