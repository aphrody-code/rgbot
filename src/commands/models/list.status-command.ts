...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("rgbot models auth setup-token")}\` or \`${formatCliCommand("rgbot configure")}\`.`
          : `Run \`${formatCliCommand("rgbot configure")}\` or set an API key env var.`;
