...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveRgBotAgentDir: vi.fn().mockReturnValue("/tmp/rgbot-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveRgBotAgentDir: mocks.resolveRgBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/rgbot-agent/auth-profiles.json");
