...
import { readConfigFileSnapshot, type RgBotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: RgBotConfig, model: string): RgBotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("rgbot plugins install")}\`.`,
    );
  }
