...
function resolveConfiguredModelRaw(cfg: RgBotConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: RgBotConfig): string[] {
...
export function applyPrimaryModel(cfg: RgBotConfig, model: string): RgBotConfig {
...
export function applyModelAllowlist(cfg: RgBotConfig, models: string[]): RgBotConfig {
  const defaults = cfg.agents?.defaults;
