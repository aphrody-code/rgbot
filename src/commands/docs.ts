...
const SEARCH_TOOL = "https://docs.molt.bot/mcp.SearchRgBot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('rgbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.molt.bot/");
      runtime.log(`Search: ${formatCliCommand('rgbot docs "your query"')}`);
    }
    return;
  }
