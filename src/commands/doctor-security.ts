...
import type { RgBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: RgBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("rgbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("rgbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("rgbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("rgbot doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "rgbot config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("rgbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
