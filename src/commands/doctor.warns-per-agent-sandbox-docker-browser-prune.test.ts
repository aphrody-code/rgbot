...
beforeEach(() => {
  ...
  resolveRgBotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "rgbot-doctor-state-"));
...
const resolveRgBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadRgBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  ...
  it("warns when extra workspace directories exist", async () => {
    ...
    const legacyPath = path.join("/Users/steipete", "rgbot");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/rgbot" || value === legacyPath || value === legacyAgentsPath)
        return true;
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Extra workspace" && typeof message === "string" && message.includes("rgbot"),
      ),
    ).toBe(true);
