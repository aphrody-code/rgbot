...
export async function maybeMigrateLegacyGatewayService(
  ...
  const migrate = await prompter.confirmSkipInNonInteractive({
    message: "Migrate legacy gateway services to RgBot now?",
    initialValue: true,
  });
  if (!migrate) return;
  ...
  if (loaded) {
    note(`RgBot ${service.label} already ${service.loadedText}.`, "Gateway");
    return;
  }

  const install = await prompter.confirmSkipInNonInteractive({
    message: "Install RgBot gateway service now?",
    initialValue: true,
  });
  if (!install) return;
