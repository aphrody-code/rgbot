...
function stripUnknownConfigKeys(config: RgBotConfig): {
  config: RgBotConfig;
  removed: string[];
} {
  const parsed = RgBotSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as RgBotConfig;
...
function noteOpencodeProviderOverrides(cfg: RgBotConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "clawdbot.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "rgbot.json");
  ...
  let cfg: RgBotConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as RgBotConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("rgbot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("rgbot doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "rgbot doctor --fix" to remove these keys.');
    }
  }
