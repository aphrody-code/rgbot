...
export async function statusCommand(
  ...
  runtime.log(theme.heading("RgBot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("rgbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("rgbot security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("rgbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("rgbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("rgbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("rgbot gateway probe")}`);
  }
}
