...
export function listAgentEntries(cfg: RgBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: RgBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: RgBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: RgBotConfig): AgentSummary[] {
