...
vi.mock("../infra/rgbot-root.js", () => ({
  resolveRgBotPackageRoot: vi.fn().mockResolvedValue("/tmp/rgbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/rgbot/pnpm-lock.yaml",
      markerPath: "/tmp/rgbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("RgBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("rgbot status --all") ||
          l.includes("rgbot --profile isolated status --all") ||
          l.includes("rgbot status --all") ||
          l.includes("rgbot --profile isolated status --all"),
      ),
    ).toBe(true);
