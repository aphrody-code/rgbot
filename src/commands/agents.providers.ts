...
function resolveDefaultAccountId(cfg: RgBotConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: RgBotConfig): boolean {
...
export function summarizeBindings(cfg: RgBotConfig, bindings: AgentBinding[]): string[] {
