...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("rgbot doctor")}" (or "${formatCliCommand("rgbot doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("rgbot gateway install --force")}\` from the same --profile / CLAWDBOT_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("rgbot gateway install")}`),
    );
    spacer();
  }
  ...
  if (legacyServices.length > 0) {
    ...
    defaultRuntime.error(errorText(`Cleanup: ${formatCliCommand("rgbot doctor")}`));
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("rgbot status")}`);
