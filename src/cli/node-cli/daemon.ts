...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("rgbot node install"), formatCliCommand("rgbot node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("rgbot node install --force")}`);
    }
    return;
  }
