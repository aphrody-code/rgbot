...
export function registerDnsCli(program: Command) {
  ...
  dns
    .command("setup")
    .description("Set up CoreDNS to serve rgbot.internal for unicast DNS-SD (Wide-Area Bonjour)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.clawdbot/rgbot.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): rgbot.internal"));
      ...
      const serverPath = path.join(confDir, "rgbot.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by rgbot dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.clawdbot/rgbot.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
