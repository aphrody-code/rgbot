...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["rgbot status", "Show channel health + session summary."],
          ["rgbot status --all", "Full diagnosis (read-only)."],
          ["rgbot status --json", "Machine-readable output."],
          ["rgbot status --usage", "Show model provider usage/quota snapshots."],
          [
            "rgbot status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["rgbot status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
  ...
  program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["rgbot sessions", "List all sessions."],
          ["rgbot sessions --active 120", "Only last 2 hours."],
          ["rgbot sessions --json", "Machine-readable output."],
          ["rgbot sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
