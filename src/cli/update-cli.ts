...
import { resolveRgBotPackageRoot } from "../infra/rgbot-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "rgbot doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "rgbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "rgbot"]);
const CLI_NAME = resolveCliName();
const CLAWDBOT_REPO_URL = "https://github.com/rgbot/rgbot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "rgbot");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("rgbot@")) return trimmed.slice("rgbot@".length);
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CLAWDBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDBOT_GIT_DIR to an empty folder or a rgbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("RgBot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating RgBot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this RgBot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("rgbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("rgbot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g rgbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g rgbot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("rgbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("rgbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("rgbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("rgbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `rgbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `CLAWDBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDBOT_GIT_DIR to an empty folder or a rgbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update RgBot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["rgbot update", "Update a source checkout (git)"],
        ["rgbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["rgbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["rgbot update --tag beta", "One-off update to a dist-tag or version"],
        ["rgbot update --no-restart", "Update without restarting the service"],
        ["rgbot update --json", "Output result as JSON"],
        ["rgbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["rgbot update wizard", "Interactive update wizard"],
        ["rgbot --update", "Shorthand for rgbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run rgbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["rgbot update status", "Show channel + version status."],
          ["rgbot update status --json", "JSON output."],
          ["rgbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
