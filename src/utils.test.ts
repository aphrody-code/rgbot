...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "rgbot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "rgbot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "rgbot-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "rgbot-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.rgbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "rgbot-config-dir-"));
    try {
