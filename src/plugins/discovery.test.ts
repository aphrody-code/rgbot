...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `rgbot-plugins-${randomUUID()}`);
...
describe("discoverRgBotPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverRgBotPlugins } = await import("./discovery.js");
      return discoverRgBotPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        rgbot: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@rgbot/voice-call",
        rgbot: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@rgbot/demo-plugin-dir",
        rgbot: { extensions: ["./index.js"] },
      }),
      "utf-8",
    );
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverRgBotPlugins } = await import("./discovery.js");
      return discoverRgBotPlugins({ extraPaths: [packDir] });
    });
