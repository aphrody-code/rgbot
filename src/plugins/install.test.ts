...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `rgbot-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.clawdbot/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        rgbot: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without rgbot.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@rgbot/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("rgbot.extensions");
  });
});
