...
import { discoverRgBotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: RgBotPluginDefinition;
  register?: RgBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as RgBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as RgBotPluginDefinition;
...
export function loadRgBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverRgBotPlugins({
    workspaceDir: options.workspaceDir,
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            ...
            "rgbot/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: RgBotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as RgBotPluginModule;
    } catch (err) {
