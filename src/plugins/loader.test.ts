...
import { loadRgBotPlugins } from "./loader.js";
...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `rgbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "rgbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
describe("loadRgBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadRgBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadRgBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@rgbot/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        rgbot: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
