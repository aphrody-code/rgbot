...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { rgbot?: { emoji?: string } };
    expect(parsed.rgbot?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { rgbot?: { events?: string[] } };
