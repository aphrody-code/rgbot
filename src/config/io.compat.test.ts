...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".rgbot" | ".clawdbot",
  port: number,
  filename: "rgbot.json" | "clawdbot.json" = "rgbot.json",
) {
...
describe("config io compat (new + legacy folders)", () => {
  it("prefers ~/.rgbot/rgbot.json when both configs exist", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".rgbot", 19001);
  ...
  it("falls back to ~/.clawdbot/rgbot.json when only legacy exists", async () => {
    await withTempHome(async (home) => {
  ...
  it("prefers rgbot.json over legacy filename in the same dir", async () => {
    await withTempHome(async (home) => {
      const preferred = await writeConfig(home, ".clawdbot", 20003, "rgbot.json");
  ...
  it("honors explicit legacy config path env override", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".rgbot", 19002);
