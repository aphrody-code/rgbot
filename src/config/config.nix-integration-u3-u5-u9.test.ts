...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("CONFIG_PATH defaults to ~/.clawdbot/rgbot.json when env not set", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.clawdbot[\\/]rgbot\.json$/);
        },
      );
    });
    ...
    it("CONFIG_PATH respects CLAWDBOT_CONFIG_PATH override", async () => {
      await withEnvOverride(
        { MOLTBOT_CONFIG_PATH: undefined, CLAWDBOT_CONFIG_PATH: "/nix/store/abc/rgbot.json" },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/abc/rgbot.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH prefers MOLTBOT_CONFIG_PATH over legacy override", async () => {
      await withEnvOverride(
        {
          MOLTBOT_CONFIG_PATH: "/nix/store/new/rgbot.json",
          CLAWDBOT_CONFIG_PATH: "/nix/store/legacy/rgbot.json",
        },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/new/rgbot.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "rgbot.json"));
        },
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        ...
        await fs.writeFile(
          path.join(pluginDir, "rgbot.plugin.json"),
          JSON.stringify(
            {
              id: "demo-plugin",
        ...
        await fs.writeFile(
          path.join(configDir, "rgbot.json"),
          JSON.stringify(
            {
              plugins: {
                load: {
