...
import type { RgBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareRgBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): RgBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as RgBotConfig;
}
...
function stampConfigVersion(cfg: RgBotConfig): RgBotConfig {
...
function warnIfConfigFromFuture(cfg: RgBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareRgBotVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer RgBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: RgBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): RgBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as RgBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as RgBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: RgBotConfig) {
    clearConfigCache();
...
export function loadConfig(): RgBotConfig {
...
export async function writeConfigFile(cfg: RgBotConfig): Promise<void> {
  clearConfigCache();
