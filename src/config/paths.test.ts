...
describe("state + config path candidates", () => {
  ...
  it("orders default config candidates as new then legacy", () => {
    ...
    expect(candidates[0]).toBe(path.join(home, ".rgbot", "rgbot.json"));
    expect(candidates[1]).toBe(path.join(home, ".rgbot", "clawdbot.json"));
    expect(candidates[2]).toBe(path.join(home, ".clawdbot", "rgbot.json"));
    expect(candidates[3]).toBe(path.join(home, ".clawdbot", "clawdbot.json"));
  });

  it("prefers ~/.rgbot when it exists and legacy dir is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-state-"));
    try {
      const newDir = path.join(root, ".rgbot");
  ...
  it("CONFIG_PATH prefers existing legacy filename when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-config-"));
    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousHomeDrive = process.env.HOMEDRIVE;
    const previousHomePath = process.env.HOMEPATH;
    const previousRgBotConfig = process.env.MOLTBOT_CONFIG_PATH;
    const previousClawdBotConfig = process.env.CLAWDBOT_CONFIG_PATH;
    const previousRgBotState = process.env.MOLTBOT_STATE_DIR;
    ...
    try {
      ...
      if (previousRgBotConfig === undefined) delete process.env.MOLTBOT_CONFIG_PATH;
      else process.env.MOLTBOT_CONFIG_PATH = previousRgBotConfig;
      ...
      if (previousRgBotState === undefined) delete process.env.MOLTBOT_STATE_DIR;
      else process.env.MOLTBOT_STATE_DIR = previousRgBotState;
  ...
  it("respects state dir overrides when config is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-config-override-"));
    try {
      const legacyDir = path.join(root, ".clawdbot");
      await fs.mkdir(legacyDir, { recursive: true });
      const legacyConfig = path.join(legacyDir, "rgbot.json");
      await fs.writeFile(legacyConfig, "{}", "utf-8");

      const overrideDir = path.join(root, "override");
      const env = { MOLTBOT_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const resolved = resolveConfigPath(env, overrideDir, () => root);
      expect(resolved).toBe(path.join(overrideDir, "rgbot.json"));
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
