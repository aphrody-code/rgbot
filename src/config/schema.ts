...
export type ConfigSchema = ReturnType<typeof RgBotSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when RgBot writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /rgbot).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `rgbot plugins update` to locate install sources).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/rgbot",
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = RgBotSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "RgBotConfig";
