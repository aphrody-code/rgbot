...
import type { RgBotConfig, ConfigValidationIssue } from "./types.js";
import { RgBotSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: RgBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: RgBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = RgBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as RgBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as RgBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as RgBotConfig)),
    ),
  };
}
