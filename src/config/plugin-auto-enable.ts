import type { RgBotConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: RgBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: RgBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: RgBotConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: RgBotConfig): boolean {
...
function isIMessageConfigured(cfg: RgBotConfig): boolean {
...
function isWhatsAppConfigured(cfg: RgBotConfig): boolean {
...
function isGenericChannelConfigured(cfg: RgBotConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: RgBotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: RgBotConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: RgBotConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: RgBotConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: RgBotConfig, pluginId: string): RgBotConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
