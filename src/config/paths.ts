...
const NEW_STATE_DIRNAME = ".rgbot";
const CONFIG_FILENAME = "rgbot.json";
 ...
 * If ~/.rgbot exists and ~/.clawdbot does not, prefer ~/.rgbot.
 */
 ...
 * Default: ~/.clawdbot/rgbot.json (or $*_STATE_DIR/rgbot.json)
 */
...
export function resolveDefaultConfigCandidates(
  ...
  const rgbotStateDir = env.MOLTBOT_STATE_DIR?.trim();
  if (rgbotStateDir) {
    candidates.push(path.join(resolveUserPath(rgbotStateDir), CONFIG_FILENAME));
    candidates.push(path.join(resolveUserPath(rgbotStateDir), LEGACY_CONFIG_FILENAME));
  }
 ...
 * Default: os.tmpdir()/rgbot-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `rgbot-${uid}` : "rgbot";
