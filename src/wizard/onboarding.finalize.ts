...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: RgBotConfig;
  nextConfig: RgBotConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.clawdbot/rgbot.json (gateway.auth.token) or CLAWDBOT_GATEWAY_TOKEN.",
        "Web UI stores a copy in this browser's localStorage (rgbot.control.settings.v1).",
        `Get the tokenized link anytime: ${formatCliCommand("rgbot dashboard --no-open")}`,
      ].join("\n"),
      "Token",
    );
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control RgBot."
            : "Copy/paste this URL in a browser on this machine to control RgBot.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("rgbot dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          "RgBot uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("rgbot configure --section web")}`,
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened with your token; keep that tab to control RgBot."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the tokenized link above."
        : "Onboarding complete. Use the tokenized dashboard link above to control RgBot.",
  );
}
