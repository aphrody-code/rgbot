...
export function listMattermostAccountIds(cfg: Rose Griffon BotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: Rose Griffon BotConfig): string {
...
export function listEnabledMattermostAccounts(cfg: Rose Griffon BotConfig): ResolvedMattermostAccount[] {
