...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZaloDmPolicy(cfg as Rose Griffon BotConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZaloAccountId(cfg as Rose Griffon BotConfig);
...
export const zaloOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const configured = listZaloAccountIds(cfg as Rose Griffon BotConfig).some((accountId) =>
      Boolean(resolveZaloAccount({ cfg: cfg as Rose Griffon BotConfig, accountId }).token),
    );
    return {
      channel,
      configured,
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultZaloAccountId = resolveDefaultZaloAccountId(cfg as Rose Griffon BotConfig);
