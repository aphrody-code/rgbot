...
async function promptZalouserAllowFrom(params: {
  ...
  accountId: string;
}): Promise<Rose Griffon BotConfig> {
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  getCurrent: (cfg) => ((cfg as Rose Griffon BotConfig).channels?.zalouser?.dmPolicy ?? "pairing") as "pairing",
  setPolicy: (cfg, policy) => setZalouserDmPolicy(cfg as Rose Griffon BotConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZalouserAccountId(cfg as Rose Griffon BotConfig);
...
export const zalouserOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const ids = listZalouserAccountIds(cfg as Rose Griffon BotConfig);
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultAccountId = resolveDefaultZalouserAccountId(cfg as Rose Griffon BotConfig);
    ...
    let next = cfg as Rose Griffon BotConfig;
