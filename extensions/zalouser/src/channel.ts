...
function resolveZalouserGroupToolPolicy(
  ...
  const account = resolveZalouserAccountSync({
    cfg: params.cfg as Rose Griffon BotConfig,
...
export const zalouserDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZalouserAccountSync({ cfg: cfg as Rose Griffon BotConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZalouserAccountIds(cfg as Rose Griffon BotConfig),
    resolveAccount: (cfg, accountId) =>
      resolveZalouserAccountSync({ cfg: cfg as Rose Griffon BotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZalouserAccountId(cfg as Rose Griffon BotConfig),
    setAccountEnabled: ({ cfg, accountId, enabled }) =>
      setAccountEnabledInConfigSection({
        cfg: cfg as Rose Griffon BotConfig,
        sectionKey: "zalouser",
        accountId,
        enabled,
        allowTopLevel: true,
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as Rose Griffon BotConfig).channels?.zalouser?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    self: async ({ cfg, accountId, runtime }) => {
      ...
      const account = resolveZalouserAccountSync({ cfg: cfg as Rose Griffon BotConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZalouserAccountSync({ cfg: cfg as Rose Griffon BotConfig });
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      return monitorZalouserProvider({
        account,
        config: ctx.cfg as Rose Griffon BotConfig,
        runtime: ctx.runtime,
