...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["rgbotPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["rgbot", "pollId"]) ??
    readNestedString(value, ["rgbot", "poll", "id"]) ??
    readNestedString(value, ["data", "rgbotPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "rgbot", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "rgbot poll vote",
          displayText: "Vote recorded",
          value: { rgbotPollId: pollId },
        },
      },
    ],
  };
