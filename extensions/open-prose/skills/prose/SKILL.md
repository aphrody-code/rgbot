...
metadata: {"rgbot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
## Rose Griffon Bot Runtime Mapping

- **Task tool** in the upstream spec == Rose Griffon Bot `sessions_spawn`
- **File I/O** == Rose Griffon Bot `read`/`write`
- **Remote fetch** == Rose Griffon Bot `web_fetch` (or `exec` with curl when POST is required)
