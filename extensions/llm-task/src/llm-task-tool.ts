...
// NOTE: This extension is intended to be bundled with Rose Griffon Bot.
// When running from source (tests/dev), Rose Griffon Bot internals live under src/.
...
export function createLlmTaskTool(api: Rose Griffon BotPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "rgbot-llm-task-"));
