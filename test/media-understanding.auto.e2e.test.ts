...
import type { RgBotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("rgbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("rgbot-bin-sherpa-");
      const modelDir = await makeTempDir("rgbot-sherpa-model-");
      ...
      const cfg: RgBotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("rgbot-bin-whispercpp-");
      const modelDir = await makeTempDir("rgbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("rgbot-bin-gemini-");
      ...
      const cfg: RgBotConfig = { tools: { media: { image: {} } } };
